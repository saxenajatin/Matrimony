-- Custom username/password auth (NO Supabase Auth).
-- All objects use AMVS_ prefix.

-- Drop Phase-1 Supabase-Auth-oriented objects if they exist
...
comment on table public."AMVS_Users" is
  'Custom auth users. Passwords stored as bcrypt hashes only.';
comment on table public."AMVS_Sessions" is
  'Opaque session tokens (hashed). No Supabase Auth dependency.';
...
-- Role check (server-side / security definer)
...
-- Register user (password already bcrypt-hashed by app server)
...
-- Fetch active user auth row by username (for server-side password verify)
...
-- Create session (token already hashed by app server)
...
-- Change password (hash provided by app server)
...
-- App uses service-role on the server after validating custom sessions.
...
-- service_role bypasses RLS; grants kept for clarity in tooling
